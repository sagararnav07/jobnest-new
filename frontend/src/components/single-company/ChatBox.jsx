...
import { getConversation, getConversations, sendMessage } from "../../services/chat.service";
...
const ChatBox = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (chatId != null && currentUser?.email) {
        ...
        const thread = await getConversation(chatUser.email);
        setMessages(thread || []);
      }
      setIsLoading(false);
    };
  ...
  const handleSendMessage = async (e) => {
    e.preventDefault();
    if (value.trim() === "" || !chatUser?.email || !currentUser) {
      return;
    }
    const payload = {
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      try {
        ...
        setUsersCollection(conversations);
      } catch (error) {
                  ...
                  <div className="flex sm:items-center justify-between py-3 border-b-2 border-gray-200">
                    <div className="relative flex items-center space-x-4">
                      ...
                      <div className="relative">
                        {chatUser && (
                          <img
                            src={chatUser.photoURL || defaultAvatar}
                            alt=""
                            className="w-16 "
                          />
                        )}
