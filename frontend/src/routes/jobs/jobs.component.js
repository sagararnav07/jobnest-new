...
import { getJobs, searchJobs } from "../../services/jobs.service";
...
function Jobs() {
  ...
  const loadDetail = async () => {
    ...
    setIsLoading(true);
    try {
      const jobResults = await searchJobs(search);
      setJobs(jobResults || []);
    } catch (error) {
  ...
  useEffect(() => {
    const updateJobs = async (sectionId, optionValue) => {
      ...
      if (isChecked) {
        setIsLoading(true);
        try {
          // Use searchJobs with the appropriate query parameters
          const query = `${sectionId}:${optionValue}`;
